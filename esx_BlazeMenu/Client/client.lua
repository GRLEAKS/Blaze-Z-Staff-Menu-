...
--[[ Vehicle Plates ]]
  ...
  function GeneralMenu()
	...
	BlazePlayerList = Blazepool:AddSubMenu(BlazeStart,"💥 Online Players", "~r~List of the online players",(true))

	BlazePlayers = Blazepool:AddSubMenu(BlazeStart,"👨 Player Options","~r~Staff utilities for Players",(true))

	BlazePunishment = Blazepool:AddSubMenu(BlazeStart,"🚫 Punishment Options","~r~Punishment Option For Server Players",(true))

	BlazeVehicle = Blazepool:AddSubMenu(BlazeStart,"🔧 Vehicle Options","~r~Staff utilities for Vehicles",(true))

	BlazeTeleports = Blazepool:AddSubMenu(BlazeStart, "📌 Misc Options", "~r~Teleport Locations and options",(true))
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	local seeinventory = NativeUI.CreateItem("~q~Show Inventory", "~g~Enter Player id that you want to see his inventory")

    BlazePlayers:AddItem(seeinventory)
...
elseif item == seeinventory then
	...
	local continue = true
		...
		--	TriggerEvent('esx_inventoryhud:openPlayerInventory', tonumber(id))
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(0,Keys["BACKSPACE"]) then
			...
			if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'Oxhmata') then

				ESX.UI.Menu.Close('default', GetCurrentResourceName(), 'Oxhmata')
